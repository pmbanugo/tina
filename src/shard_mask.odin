...
test_shard_mask_empty_state :: proc(t: ^testing.T) {
	...
	for i in 0 ..= 255 {
		testing.expectf(t, !shard_mask_contains(&mask, Shard_Id(i)), "Bit %d should be empty initially", i)
	}
}
...
test_shard_mask_isolation :: proc(t: ^testing.T) {
	...
	// Set bit 42
	shard_mask_include(&mask, Shard_Id(42))

	// Prove exactly bit 42 is set
	testing.expect(t, shard_mask_contains(&mask, Shard_Id(42)), "Bit 42 should be set")

	// Prove adjacent bits did not flip (isolation)
	testing.expect(t, !shard_mask_contains(&mask, Shard_Id(41)), "Bit 41 should not bleed")
	testing.expect(t, !shard_mask_contains(&mask, Shard_Id(43)), "Bit 43 should not bleed")

	// Clear bit 42
	shard_mask_exclude(&mask, Shard_Id(42))
	testing.expect(t, !shard_mask_contains(&mask, Shard_Id(42)), "Bit 42 should be cleared")
}
...
test_shard_mask_bucket_boundaries :: proc(t: ^testing.T) {
	...
	boundaries := []Shard_Id{0, 63, 64, 127, 128, 191, 192, 255}
	...
	for b in boundaries {
		...
		if b > 0 {
			testing.expectf(
				t,
				!shard_mask_contains(&mask, Shard_Id(b - 1)),
				"Bit %d should not bleed down",
				b,
			)
		}
		if b < 255 {
			testing.expectf(t, !shard_mask_contains(&mask, Shard_Id(b + 1)), "Bit %d should not bleed up", b)
		}
...
test_shard_mask_all_alive_default :: proc(t: ^testing.T) {
	...
	for i in 0 ..= 255 {
		testing.expectf(
			t,
			shard_mask_contains(&mask, Shard_Id(i)),
	...
	shard_mask_exclude(&mask, Shard_Id(150))
...
test_shard_mask_max_capacity :: proc(t: ^testing.T) {
	...
	for i in 0 ..= 255 {
		shard_mask_include(&mask, Shard_Id(i))
	}
