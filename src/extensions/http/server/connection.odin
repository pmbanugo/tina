...
_connection_working_memory_bytes :: proc(ctx: tina.TinaContext) -> []u8 {
	return tina.ctx_working_arena_bytes(ctx)
}
...
_http_connection_handler :: proc(
	...
	case tina.IO_TAG_CLOSE_COMPLETE:
		_connection_complete_close(connection, ctx)
...
_connection_process_header_bytes :: proc(
	...
	case .Error_Bad_Request,
	     ...
	     .Error_Version:
		return _connection_send_parse_error(connection, parse_status, ctx)
...
_connection_send_parse_error :: proc(
	...
	return _connection_stage_canned_response(connection, transmute([]u8)bytes, ctx)
}
...
_connection_make_response :: proc(
	...
	return Response{connection = connection, tina_context = ctx}
}
...
test_connection_make_request_uses_retained_frame_when_frame_absent :: proc(t: ^testing.T) {
	...
	request := _connection_make_request(&fixture.connection, nil, 0)
	...
	if descriptor.body_mode == .Streamed {
		for {
			...
			if result.protocol_error {
				return _connection_send_parse_error(connection, .Error_Bad_Request, ctx)
			}
...
@(private = "file")
_connection_begin_close :: proc(connection: ^HTTP_Connection, ctx: tina.TinaContext) -> tina.Effect {
	state := &connection.connection_state
	runtime := state.shard_runtime
	if runtime != nil {
...
@(private = "file")
_connection_complete_close :: proc(connection: ^HTTP_Connection, ctx: tina.TinaContext) {
	state := &connection.connection_state
	runtime := state.shard_runtime
	if runtime == nil {
		return
	...
	_idle_slot_remove(connection, ctx)
	if _runtime_active_slot_remove(connection, ctx) && runtime.free_count < runtime.connection_slot_count {
...
test_idle_peer_close_unlinks_idle_before_fd_close :: proc(t: ^testing.T) {
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			test_state := cast(^Close_Lifecycle_Test_State)user_data
			effect := _http_connection_handler(rawptr(test_state.connection), test_state.message, ctx)
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			test_state := cast(^Close_Lifecycle_Test_State)user_data
			_ = _http_connection_handler(rawptr(test_state.connection), test_state.message, ctx)
			_ = _http_connection_handler(rawptr(test_state.connection), test_state.message, ctx)
		},
	)
...
test_stale_notification_on_keep_alive_connection_is_dropped :: proc(t: ^testing.T) {
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			#partial switch _ in park_effect {
			case tina.Effect_Receive:
			case:
				testing.expect(test_state.t, false, "Request_Start should park for notifications")
			}
			...
			#partial switch _ in stale_effect {
			case tina.Effect_Receive:
			case:
				testing.expect(
					test_state.t,
					false,
					"stale notification should be dropped and re-parked",
				)
			}
			...
			#partial switch _ in correct_effect {
			case tina.Effect_Receive:
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			case:
				testing.expect(
					test_state.t,
					false,
					"message outside Application_Expectation should be preserved and deferred",
				)
			}
			testing.expect(
				test_state.t,
			...
			case:
				testing.expect(
					test_state.t,
					false,
					"deferred notification should produce close effect from handler",
				)
			}
		},
	)
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			case:
				testing.expect(
					test_state.t,
					false,
					"first deferred notification should dispatch before later arrivals",
				)
			}
		},
	)
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			case:
				testing.expect(
					test_state.t,
					false,
					"shutdown during Application_Expectation should dispatch Server_Drain",
				)
			}
		},
	)
...
test_dispatch_step_flush_non_final_dispatches_send_ready_without_io :: proc(t: ^testing.T) {
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			case:
				testing.expect(test_state.t, false, "expected close after Send_Ready handler")
			}
		},
	)
...
test_send_complete_with_sendfile_plan_transitions_to_sendfile_io :: proc(t: ^testing.T) {
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			case:
				testing.expect(test_state.t, false, "expected Effect_Io")
			}
		},
	)
}
...
test_stage_canned_response_arms_send_timeout :: proc(t: ^testing.T) {
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			case:
				testing.expect(state.t, false, "expected Effect_Io")
			}
		},
	)
...
test_drive_body_read_restarts_send_from_unsent_offset :: proc(t: ^testing.T) {
	...
	tina.test_with_context(
		...
		proc(user_data: rawptr, ctx: tina.TinaContext) {
			...
			#partial switch io_effect in effect {
			case tina.Effect_Io:
				#partial switch send_op in io_effect.operation {
				case tina.IoOp_Send:
