...
@(private = "package")
_backend_collect :: proc(backend: ^Platform_Backend, completions: []Raw_Completion, timeout_ns: i64) -> (u32, Backend_Error) {
	out: u32 = 0
	...
	for i in 0 ..< n {
		...
		sub := Submission{token = pop.token, operation = pop.operation}
		result, immediate := _try_syscall(&sub)
...
_backend_control_setsockopt :: proc(
	...
	plevel: c.int
	switch level {
	case .SOL_SOCKET:   plevel = c.int(posix.SOL_SOCKET)
	case .IPPROTO_TCP:  plevel = 6
	case .IPPROTO_UDP:  plevel = 17
	case .IPPROTO_IPV6: plevel = 41
	}
...
_backend_control_shutdown :: proc(backend: ^Platform_Backend, fd: OS_FD, how: Shutdown_How) -> Backend_Error {
	phow: posix.Shut
	switch how {
	case .SHUT_READER: phow = .RD
	case .SHUT_WRITER: phow = .WR
	case .SHUT_BOTH:   phow = .RDWR
	}
...
_try_syscall :: proc(sub: ^Submission) -> (Raw_Completion, bool) {
	result := Raw_Completion{token = sub.token, extra = nil}

	switch op in sub.operation {
	case Submission_Op_Read:
		n := posix.pread(posix.FD(op.fd), ([^]byte)(op.buffer), uint(op.size), posix.off_t(op.offset))
		if n < 0 {
			errno := posix.errno()
	...
	case Submission_Op_Write:
		n := posix.pwrite(posix.FD(op.fd), ([^]byte)(op.buffer), uint(op.size), posix.off_t(op.offset))
		if n < 0 {
			errno := posix.errno()
			if errno == .EWOULDBLOCK || errno == .EAGAIN {
				return result, false
	...
	case Submission_Op_Accept:
		...
		client_fd := posix.accept(posix.FD(op.listen_fd), (^posix.sockaddr)(&client_addr), &addr_len)
		if client_fd < 0 {
			errno := posix.errno()
			if errno == .EWOULDBLOCK || errno == .EAGAIN {
				return result, false
	...
	case Submission_Op_Sendto:
		sa, sa_len := _socket_address_to_sockaddr(op.address)
		n := posix.sendto(posix.FD(op.socket_fd), rawptr(op.buffer), uint(op.size), {.NOSIGNAL}, (^posix.sockaddr)(&sa), sa_len)
		if n < 0 {
			errno := posix.errno()
			if errno == .EWOULDBLOCK || errno == .EAGAIN {
				return result, false
	...
	case Submission_Op_Recvfrom:
		...
		n := posix.recvfrom(posix.FD(op.socket_fd), rawptr(op.buffer), uint(op.size), {}, (^posix.sockaddr)(&peer_addr), &addr_len)
		if n < 0 {
			errno := posix.errno()
			if errno == .EWOULDBLOCK || errno == .EAGAIN {
				return result, false
...
@(private = "file")
_socket_address_to_sockaddr :: proc(addr: Socket_Address) -> (posix.sockaddr_storage, posix.socklen_t) {
...
_map_socket_option :: proc(option: Socket_Option) -> (posix.Sock_Option, bool) {
	...
	case .SO_REUSEADDR: return .REUSEADDR, true
	case .SO_KEEPALIVE: return .KEEPALIVE, true
	case .SO_RCVBUF:    return .RCVBUF, true
	case .SO_SNDBUF:    return .SNDBUF, true
	case .SO_LINGER:    return .LINGER, true
	case:               return .REUSEADDR, false
	}
}
